...
public class MqttClientDaily {

	  	MqttClient mqtt_client;
		MqttConnectOptions conn;
		
		static final Logger logger = Logger.getLogger(MqttClientDaily.class.getName());
		...
		public static void main(String[] args) {
			PropertyConfigurator.configure(MqttClientDaily.class.getClassLoader().getResource("edu/indiana/d2i/mqtt/log4j.properties"));
	        ...
	        MqttClientDaily mcd = new MqttClientDaily();
	        mcd.runClient();
		}
		 ...
		 * Create a MQTT client, connect to broker, publisher/subscriber, disconnect.
		 * 
		 */
		            	...
		            	if(topic.contains("LASS/MAPS_COPY")){
		            		...
		            		for (String val : value_split) {
		            			if(val.contains("device_id")){
			            	        ...
			            	        File dataFile = new File(year + "_D" + day_num + "_" + device_id + ".tmp");
			            		    ...
			            		    for (int i = 0; i < listOfFolders.length; i++) {
			            		        if (listOfFolders[i].isFile()) {
			            			        ...
			            			        if(!(file_name.contains(year + "_D" + day_num)) && file_name.contains(".tmp") && file_name.contains(year + "_D")){
